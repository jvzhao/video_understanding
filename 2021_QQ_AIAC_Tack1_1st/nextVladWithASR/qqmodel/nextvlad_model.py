...
class NextVladModel(nn.Module):
    def __init__(self, cfg, bert_cfg_dict, model_path, init_from_pretrain=True) -> None:
        ...
        self.nn1 = nn.Linear(3072, 10000)
        self.nn2 = nn.Linear(10000, 10000)

        # for param in self.roberta.parameters():
        #     param.requires_grad = False

    def forward(self, video_feature, text_input_ids, text_mask, asr_ids, asr_mask, target):
        ...
        asr_features = self.roberta(asr_ids, asr_mask)
        asr_mean = torch.mean(asr_features, 1)
        # print("video_features:",video_features.shape)
        # print("text_features",text_features.shape)
        # print("embedding_feature", embedding.shape)
        features = torch.cat([features_mean,asr_mean, video_features], 1)
        pred = F.relu(self.nn1(features))
        pred = F.sigmoid(self.nn2(pred))
