...
class record_transform():
    def __init__(self, model_path, tag_file, 
                 ...
                 text_maxlen=52, frame_maxlen=32):
    ...
    def transform(self, features, parse=True):
        ...
        if self.get_asr:
            o['asr_id'] = torch.tensor(features['asr_id'], dtype=torch.long)
            o['asr_mask'] = torch.tensor(features['asr_mask'], dtype=torch.long)
        if self.get_frame:
    ...
    def parse_tfrecord(self, features):
        ...
        if self.get_asr:
            text = bytes(np.frombuffer(features['asr_text'], dtype=np.uint8)).decode()
            if text == "nan" or text == "None":
                text = ''
            asr_id_mask = self.tokenizer.encode_plus(
                                ['', text],
                                add_special_tokens=True,
                                max_length=self.text_maxlen,
                                padding='max_length',
            ...
            o['asr_id'] = asr_id_mask['input_ids']# [CLS][SEP] Text [SEP]
            o['asr_mask'] = asr_id_mask['attention_mask']
            o['asr_text'] = text
